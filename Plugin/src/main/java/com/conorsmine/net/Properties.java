...
public enum Properties {
    ...
    private static final Lazy<java.util.Properties> PROP = new Lazy<>(Properties::loadProperties);
    ...
    @Nullable
    // propVal is the ACTUAL value then, not the propPath variable!
    static Properties getPropertyFromPropertyString(final @NotNull String propVal) {
        for (Properties value : Properties.values()) {
            if (PROP.get().getProperty(value.propPath).equalsIgnoreCase(propVal)) return value;
        }
